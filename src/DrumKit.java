
import java.applet.AudioClip;
import java.awt.GridLayout;
import java.awt.event.MouseAdapter;
import java.awt.event.MouseEvent;
import java.net.MalformedURLException;
import java.net.URL;

import javax.swing.Icon;
import javax.swing.ImageIcon;
import javax.swing.JApplet;
import javax.swing.JFrame;
import javax.swing.JLabel;
import javax.swing.JPanel;

public class DrumKit extends MouseAdapter {

    JLabel drumLabelWithImage;
    JLabel image;
    JLabel image2;
	JLabel image3;
    public static void main(String[] args) throws Exception {
   	 new DrumKit().getGoing();
    }

    private void getGoing() throws MalformedURLException {
   	 // 1. Make a JFrame variable and initialize it using "new JFrame()"
   	 JFrame frameA = new JFrame();
   	 // 2. Make the frame visible
   frameA.setVisible(true);
   	 // 3. Set the size of the frame
   	 frameA.setSize(1000, 1000);
   	 // 4. Set the title of the frame
   	 frameA.setTitle("Drum Set");
   	 // 5. Make a JPanel and initialize it.
   	 JPanel panelA = new JPanel();
   	 // 6. Add the panel to the frame. (The panel is invisible.)
   frameA.add(panelA);

   	 // 7. Download an image of a drum from the Internet. Drop it into your Eclipse project under "default package".
   	 // 8. Put the name of your image in a String variable.
    String bassDrum = ("$_35.JPG");
    String snareDrum = ("snaredrum.jpg");
    String cymbals = ("cymbals.JPG");
   	 // 9. Use the "createLabelImage()" method to initialize the drumLabelWithImage variable.
   	 image = createLabelImage(bassDrum);
   	 image2 = createLabelImage(snareDrum);
   	 image3 = createLabelImage(cymbals);

   	 // 10. Add the image to the panel
    panelA.add(image);
    panelA.add(image2);
    panelA.add(image3);
   	 // 11. Set the layout of the panel to "new GridLayout()"
    panelA.setLayout(new GridLayout());
   	 // 12. call the pack() method on the frame
  frameA.pack();
   	 // 13. add a mouse listener to drumLabelWithImage.
image.addMouseListener(this);
image2.addMouseListener(this);
image3.addMouseListener(this);
    }

    public void mouseClicked(MouseEvent e) {
   	 // 14. When the mouse is clicked, print "mouse clicked"
System.out.println("Mouse Clicked");
   	 JLabel drumClicked = (JLabel) e.getSource();
   	 // 15. Download a drum sound and drop it into your "default package". There are some sounds here: http://bit.ly/drum-sounds or you can also get them from freesound.org
   	 // 16. If they clicked on the drumImage...
if(drumClicked.equals(image)){
	playSound("321132__meisterjaan__shaman-s-drum.wav");
}else if(drumClicked.equals(image2)){
	playSound("178668__hanbaal__snare.wav");
}else if(drumClicked.equals(image3)){
	playSound("209868__veiler__sabian-aax-chinese-3.wav");
}
   		 // 17. ...use the playSound method to play a drum sound.

   	 // 18. Add more images to make a drumkit. Remember to add a mouse listener to each one.
    }

    private JLabel createLabelImage(String fileName)
			throws MalformedURLException
	{
		URL imageURL = getClass().getResource(fileName);
		if (imageURL == null)
		{
			System.err.println("Could not find image " + fileName);
			return new JLabel();
		}
		Icon icon = new ImageIcon(imageURL);
		JLabel imageLabel = new JLabel(icon);
		return imageLabel;
	}


    private void playSound(String fileName) {
   	 AudioClip sound = JApplet.newAudioClip(getClass().getResource(fileName));
   	 sound.play();
    }

}



