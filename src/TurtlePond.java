...
public class TurtlePond implements KeyEventDispatcher {
	...
	void setup() {
		...
		JOptionPane.showMessageDialog(null,
				"Using the arrow keys, move the turtle to find the hidden cookie.\nThe closer you are, the warmer you'll be.\n\nMake sure she doesn't go hungry -- you only have 20 seconds!\nAlso, be careful not to cross your path!");
	...
	private void checkForFood() throws Exception {
		...
		if (Math.abs(tortoiseLocationX - cookieX) <= 5 && Math.abs(tortoiseLocationY - cookieY) <= 5) {
		...
		} else if (Math.abs(tortoiseLocationX - cookieX) <= 20 && Math.abs(tortoiseLocationY - cookieY) <= 20) {
			...
			setBackgroundColor(PenColors.Grays.SlateGray);
		...
		if (getTimeElapsed() == 20) {
			JOptionPane.showMessageDialog(null, "Oh no! The turtle died of hunger!");
			System.exit(0);
		}
		...
		if (wasHereBefore(tortoiseLocationX, tortoiseLocationY) == true) {
			...
			JOptionPane.showMessageDialog(null, "Oh no! You tried to cross your own path -- go back to the beginning!");
			Point pie = getFirstLocation();
			int originX = (int) pie.getX();
			int originY = (int) pie.getY();
			Tortoise.penUp();
			Tortoise.setX(originX);
			Tortoise.setY(originY);
			Tortoise.penDown();
